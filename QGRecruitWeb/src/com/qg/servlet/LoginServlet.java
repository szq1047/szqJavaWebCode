				...
				if (role.equals("admin")) {
					if(adminbll.Exists(userName)){
						if(adminbll.Exists(userName, passWord)){
							//管理员登录成功
							page="AdminStuff/index.jsp";
							aa = adminbll.getAdminInfo(userName,passWord);
							request.getSession().setAttribute("admin", aa);
					...
					if (userbll.Exists(userName)) {
						if (userbll.Exists(userName, passWord)) {
							...
							List status = userbll.getUserId(userName, passWord);
							request.getSession().setAttribute("userId", status.get(0));
							request.getSession().setAttribute("bmPass", status.get(1));
							request.getSession().setAttribute("testPass", status.get(2));
							request.getSession().setAttribute("isJoin", status.get(3));
