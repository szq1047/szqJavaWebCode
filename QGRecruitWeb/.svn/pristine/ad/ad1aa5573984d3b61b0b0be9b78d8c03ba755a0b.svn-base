...
import com.qg.Common.ZXBMPageDivide;
import com.qg.model.Admin;

@WebServlet("/ZXBMManage")
public class ZXBMManageServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    public ZXBMManageServlet() {
        super();
    }
	protected void doGet(HttpServletRequest request, HttpServletResponse response) 
		...
		try{
			if(Integer.parseInt(ad.getAdminTop()) != 1){
				...
				String sqlStr = "SELECT * FROM zxbminfo";
				try {
					ZXBMPageDivide dd = new ZXBMPageDivide(sqlStr);
					...
					request.getRequestDispatcher("/AdminStuff/ManageBM/getZXBM.jsp").forward(request, response);	
				} catch (Exception e) {
					e.printStackTrace();
				}
			}
