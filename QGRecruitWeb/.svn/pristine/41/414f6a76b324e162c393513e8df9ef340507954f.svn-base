...
<script type="text/javascript">
	function Testdelete(testId,testpath){
		...
		if(window.confirm(msg)){
			window.location="DeleteTestServlet?TestId="+testId+"&TestPath="+testpath+"&action=usertest";
		}
	}
</script>
</head>
<body>
	...
	<div class="div_head">
		<span> <span style="float: left;">当前位置是：试题管理->试题列表</span>
		</span>
	</div>
	<div></div>
				...
				<%for(int i=0;i<testlist.size();i++){ %>		
						<tr>
							...
							<td><a href="DownloadFileServlet?filename=<%=testlist.get(i).getTestPath()%>">下载</a>
								<a href="#" onclick="Testdelete('<%=testlist.get(i).getTestId()%>','<%=testlist.get(i).getTestPath() %>');">删除</a>
								<%if(userbll.getTestPassStatus(testlist.get(i).getTestPublisher()).equals("0")){ %>
								<a href="UpdateUserStatus?userName=<%=testlist.get(i).getTestPublisher()%>">通过考试</a>
								<%}else{ %>
									<span>已通过考试</span>
								<%} %>
							</td>
